...
const uploadToCloudinary = async (filePath, resourceType = "image", retries = 3) => {
  try {
    ...
    //console.log("result 1 url: ",result.secure_url)
    return result.secure_url;
  } catch (error) {
    console.error("Cloudinary upload failed:", error);

    if (retries > 0) {
      console.warn(`Retrying... remaining attempts: ${retries - 1}`);
      await new Promise((res) => setTimeout(res, 1000));
      return uploadToCloudinary(filePath, resourceType, retries - 1);
    }
...
export const addProduct = async (req,res) => {
  //console.log("Entering",req.body.productData)
   try{
...
export const addCategory = async (req, res) => {
  try {
    ...
    const imageUrl = result.secure_url; // store only URL
     ...
     // Save to MongoDB
    const newCategory = new Category({ name, path: name, image: imageUrl });
