...
const ProductDetails = () => {
  const { products, navigate, currency, addToCart, axios, setWishlistItems, wishlistItems, country, inrToAed } = useAppContext();
  const { id } = useParams();
        ...
        <div className="flex gap-3">
          <div className="flex flex-col gap-3">
            {product.image.map((image, index) => (
              <div
                key={index}
                onClick={() => setThumbnail(`http://localhost:4000${image}`)}
          ...
          <div className="border border-gray-500/30 max-w-100 rounded overflow-hidden max-h-100">
            <img
              src={thumbnail}
              alt="Selected product"
              className="w-full h-full object-cover image-wrapper"
            />
          </div>
        </div>
  ...
  {product.testingVideo && (
    <div className="relative w-full">
      ...
      {/* Video Player */}
      <video
        src={`http://localhost:4000${product.testingVideo}`}
