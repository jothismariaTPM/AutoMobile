...
const MyOrders = () => {
    ...
    const {currency,product,axios,country, inrToAed, backend} = useAppContext();
                ...
                <div key={index} className={`relative bg-white text-gray-500/70 ${order.items.length !== index + 1 && "border-b"} border-gray-300 flex flex-col md:flex-row md:items-center justify-between p-4 py-5 md:gap-16 w-full max-w-6xl`}>
                    <div className='flex items-center mb-4 md:mb-0'>
                        <div className='bg-primary/10 p-4 rounded-lg'>
                            <img src={`${backend}${item.product.image[0]}`} alt="" className='w-14 h-14'/>
                        </div>
                        <div className='ml-4'>
                    ...
                    <p className='text-primary text-lg font-medium'>Amount: 
                      {country === "AE" ? ` AED ${convertINRtoAED(item.product.price) * item.quantity}` : `₹${item.product.price * item.quantity}`}
                    </p>
                </div>
               ))}
            </div>
