        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={step.key} className="flex flex-col items-center">
              <div
                className={`
                  w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center rounded-full border-2 mt-2
                  ${isCompleted
              ...
              <p className={`mt-2 text-[10px] sm:text-sm font-semibold ${isCompleted ? "text-green-600" : "text-gray-300"}`}>
                {step.label}
              </p>
            </div>
          );
